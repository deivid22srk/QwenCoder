...
export function buildToolInstructions(
  ...
  let instructions =
    ...
    "7. ONLY use tool names that are explicitly defined in the TOOLS AVAILABLE list above. NEVER invent or guess tool names. If a tool is not listed, it does not exist.\n" +
    "8. You may emit multiple tool calls in one response (up to 16). Prefer batching related calls. After your tool_call blocks, STOP and wait for tool results.\n" +
    "9. Tool names may contain hyphens (e.g. list-clients). Copy the name EXACTLY as listed — do not camelCase or invent variants.\n\n";
