...
/** Normalize tool name keys for alias matching (read_file ↔ read-file). */
...
export class StreamingToolParser {
   ...
   * Registers each tool under its canonical name and a normalized alias
   * (trim + lowercase + `_` → `-`) so model name variants resolve.
   */
  setTools(tools: ToolDefinitionLike[]): void {
    ...
    for (const tool of tools) {
      ...
      const alias = normalizeToolNameKey(name);
      if (alias && alias !== name) {
        this.declaredToolNameSet.add(alias);
        this.toolByName.set(alias, tool);
      }
    }
  }
  ...
  private lookupTool(name: string): ToolDefinitionLike | undefined {
    if (!name) return undefined;
    return (
      this.toolByName.get(name) ??
      this.toolByName.get(normalizeToolNameKey(name))
    );
  }

  /** Map a model-emitted name to the canonical declared tool name when possible. */
  private resolveCanonicalToolName(name: string): string {
    const tool = this.lookupTool(name);
    if (tool) {
      const canonical = this.getToolName(tool);
  ...
  private normalizeArgumentsForTool(
    ...
    const toolProperties = this.getToolProperties(this.lookupTool(name));
    let normalized = args;
    if (
  ...
  private finalizeSuccessfulToolCall(
    ...
    } else if (this.declaredToolNameSet.size > 0) {
      // Emit the canonical declared name (e.g. read_file) even if the model
      // used a hyphen/case alias (read-file / Read_File).
      tc.name = this.resolveCanonicalToolName(tc.name);
    }
  ...
  private emitIncrementalToolCallDeltas(
    ...
    if (snapshot.name && !incremental.name) {
      ...
      incremental.name = this.resolveCanonicalToolName(snapshot.name);
    }
  ...
  private isDeclaredToolName(name: string): boolean {
    ...
    if (this.declaredToolNameSet.has(name)) return true;
    return this.declaredToolNameSet.has(normalizeToolNameKey(name));
  }
  ...
  private processToolContent(content: string, result: ParserResult): void {
    ...
    if (!t.startsWith("{") && t.includes('"')) {
      ...
      if (recovered) {
        // Same policy as closed well-formed blocks: undeclared → literal text
        if (!this.isDeclaredToolName(recovered.name)) {
          this.preserveLiteralToolCall(
            content,
            result,
            `undeclared tool name: ${recovered.name}`,
          );
          return;
        }
        if (isToolcallDebugEnabled()) {
