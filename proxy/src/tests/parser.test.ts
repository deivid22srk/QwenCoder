...
test("StreamingToolParser: rejects recovered tool call with undeclared name as text", () => {
  const parser = new StreamingToolParser(TOOLS);

  const literal =
    'name": "invented_tool", "arguments": {"path": "a.txt"}}';
  const result = parser.feed(
    `Lead <tool_call>${literal}</tool_call>`,
  );

  // Malformed recovery must not invent undeclared tools — preserve as text
  assert.strictEqual(result.toolCalls.length, 0);
  assert.ok(
    result.text.includes("<tool_call>") &&
      result.text.includes("invented_tool"),
    "should preserve undeclared recovered block as literal text",
  );
});

test("StreamingToolParser: accepts hyphen alias of declared snake_case tool name", () => {
  const parser = new StreamingToolParser(TOOLS);

  const result = parser.feed(
    '<tool_call>{"name":"read-file","arguments":{"path":"a.txt"}}</tool_call>',
  );

  assert.strictEqual(result.text, "");
  assert.strictEqual(result.toolCalls.length, 1);
  // Canonical declared name is emitted, not the model alias
