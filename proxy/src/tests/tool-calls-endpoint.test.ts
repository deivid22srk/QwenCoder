...
import {
  createStreamState,
  processChatChunk,
} from "../routes/responses/streaming.js";
...
function dataLinesFromText(text: string): string[] {
  return text
    .split("\n")
    .map((line) => line.trim())
...
test("stream: emits final chunk before the only [DONE] sentinel", async () => {
  ...
  try {
    ...
    const lines = dataLinesFromText(await res.text());
    const doneIndexes = lines
      .map((line, index) => (line === "data: [DONE]" ? index : -1))
      .filter((index) => index >= 0);
    const finishIndex = lines.findIndex((line) =>
      line.includes('"finish_reason":"stop"'),
    );

    assert.deepStrictEqual(doneIndexes, [lines.length - 1]);
    assert.ok(finishIndex >= 0, "final finish_reason chunk should be present");
    assert.ok(
      finishIndex < doneIndexes[0],
      "finish_reason must be emitted before [DONE]",
    );
  } finally {
    restore();
  }
...
test("stream: switches upstream after rate limit without closing client SSE", async () => {
  let upstreamCalls = 0;
  const restore = setupFetchMock(() => {
    upstreamCalls++;
    if (upstreamCalls === 1) {
    ...
    return createSseResponse([
      `data: ${JSON.stringify({
        choices: [{ delta: { phase: "answer", content: "Recovered" } }],
      })}`,
    ]);
  });
  ...
  try {
    ...
    const lines = dataLinesFromText(text);
    assert.strictEqual(upstreamCalls, 2);
    assert.ok(text.includes("upstream switched after retryable error"));
    assert.ok(lines.some((line) => line.includes("Recovered")));
    assert.strictEqual(
      lines.filter((line) => line === "data: [DONE]").length,
      1,
    );
  } finally {
    restore();
...
test("chat completions: rejects invalid request body with 400", async () => {
...
test("responses streaming: function call argument events include identifiers", () => {
  const state = createStreamState("resp_test", "qwen3.6-plus");
  ...
  assert.strictEqual(delta.response_id, "resp_test");
  assert.strictEqual(delta.call_id, "call_abc");
  assert.strictEqual(done.response_id, "resp_test");
  assert.strictEqual(done.call_id, "call_abc");
  assert.strictEqual(done.name, "read_file");
});
