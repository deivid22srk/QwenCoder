...
export function applyUpstreamUsage(
  ...
  // Accept Qwen (input/output) and OpenAI (prompt/completion) shapes — prefer real upstream counts
  const promptTokens =
    asFiniteNumber(usage.input_tokens) ?? asFiniteNumber(usage.prompt_tokens);
...
/** True when at least one field came from upstream (not local estimate seed). */
export function hasRealUpstreamUsage(accumulator: UsageAccumulator): boolean {
  return (
    accumulator.hasRealPromptTokens ||
    accumulator.hasRealCompletionTokens ||
    accumulator.hasRealTotalTokens
  );
}
